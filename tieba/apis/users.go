...
func GetUserForumLike(accWin8 *postbar.Account /*, uid uint64*/) ([]ForumLikePageForum, error, *postbar.PbError) {
	...
	resp, err := simple_http.Post("http://c.tieba.baidu.com/c/f/forum/like", parameters)
	if err != nil {
		return nil, err, nil
	}
...
func _GetUserProfile(acc *postbar.Account, uid uint64) {
	/*
		...
		resp, err := simple_http.Post("http://c.tieba.baidu.com/c/u/user/profile", parameters)
	*/
}
...
func GetUserInfo(acc *postbar.Account, uid uint64) (*UserInfo, error) {
	...
	resp, err := simple_http.Post("http://c.tieba.baidu.com/c/u/user/getuserinfo", parameters)
