...
func GetForumList(accAndr *postbar.Account) (*ForumList, error, *postbar.PbError) {
	...
	resp, err := simple_http.Post("http://c.tieba.baidu.com/c/f/forum/getforumlist", parameters)
...
func RMSgin(accAndr *postbar.Account, forum_ids []uint64) ([]byte, error) {
	...
	return simple_http.Post("http://c.tieba.baidu.com/c/c/forum/msign", parameters)
}
...
func Sign(accAndr *postbar.Account, fid uint64, kw, tbs string) (*SignInfo, error, *postbar.PbError) {
	...
	if _resp, err := simple_http.Post("http://c.tieba.baidu.com/c/c/forum/sign", parameters); err != nil {
		return nil, err, nil
	} else {
		resp = _resp
	}
